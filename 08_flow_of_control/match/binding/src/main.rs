...
fn main() {
    ...
    match age() {
        0 => println!("Not yet a person"),
        // Could `match` 1..= 12 directly but then what age
        // would the child be? Instead, bind to `n` for the
        // sequence of 1 ..= 12. Now the age can be reported.
        n @ 1..=12 => println!("I'm {:?} years old", n),
        n @ 13..=19 => println!("I'm {:?} years old", n),
        // Nothing bound. Return the result.
        n => println!("I'm {:?} years old", n),
    }

    // We can also use binding to "destructure" `enum` variants, such as `Option`
    match some_number() {
        // Got `Some` variant, match if its value, bound to `n`, is equal to 42
        Some(n @ 42) => println!("The answear: {}!", n),
