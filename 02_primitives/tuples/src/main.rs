...
fn main() {
    ...
    // Tuples are printable. But long Tuples (more that 12 elements) cannot be printed.
    ...
    // To create ine element tuples, the comma is required to tell them apart
    // from a literal surrounded by parentheses.
    println!("one element tuple: {:?}", (5u32,));
    println!("just an integer: {:?}", (5u32));

    // Tuples can be destructured to create bindings
    let tuple = (1, "hello", 5.4, true);
    let (a, b, c, d) = tuple;
    println!("{:?}, {:?}, {:?}, {:?}", a, b, c, d);

    let mut matrix = Matrix(1.1, 1.2, 2.1, 2.2);
    println!("Matrix:\n{}", matrix);
    println!("Matrix Reversed:\n{}", matrix.reverse());
}
